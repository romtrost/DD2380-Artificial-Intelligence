...
(:predicates
	...
	(IS-AFTERNOON ?x) ; true if lecture x is an afternoon lecture
...
(:action have-lunch 
    :parameters(?x ?y)
    :precondition(and (PERSON ?x) (BUILDING ?y) (not(had-lunch ?x)) (HAS-RESTURANT ?y) (is-person-at ?x ?y))
    :effect (had-lunch ?x)
...
(:action attend-morning-lecture
    :parameters(?x ?y ?z ?w)
    :precondition(and (IS-STUDENT ?x) (IS-TEACHER ?y) (BUILDING ?z) (LECTURE ?w)
                    (IS-MORNING ?w) (teaches-lecture ?y ?w) (IS-LECTURE-AT ?w ?z) (not(had-lunch ?x))
                    (is-person-at ?x ?z) (is-person-at ?y ?z))
...
(:action attend-afternoon-lecture
    :parameters(?x ?y ?z ?w)
    :precondition(and (IS-STUDENT ?x) (IS-TEACHER ?y) (BUILDING ?z) (LECTURE ?w)
                    (IS-AFTERNOON ?w) (teaches-lecture ?y ?w) (IS-LECTURE-AT ?w ?z) (had-lunch ?x)
                    (is-person-at ?x ?z) (is-person-at ?y ?z))
    :effect(attended-lecture ?x ?w)
	; WRITE HERE THE CODE FOR THIS ACTION
)
